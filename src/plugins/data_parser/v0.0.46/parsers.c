...
static const flag_bit_t PARSER_FLAG_ARRAY(JOB_STATE)[] = {
	...
	/* Place holders to catch masks passed as values */
	add_flag_hidden(JOB_STATE_BASE, JOB_STATE_BASE, "BASE_MASK"),
	add_flag_hidden(JOB_STATE_FLAGS, JOB_STATE_FLAGS, "STATE_FLAGS"),
...
static const parser_t PARSER_ARRAY(SLURM_CONF)[] = {
	...
	add_parse(UINT32, max_node_fail_requeue, "MaxNodeFailRequeue", "Max times a batch job may be requeued due to node failure before being held (0 = unlimited)"),
	add_parse(UINT32, max_preempt_requeue, "MaxPreemptRequeue", "Max times a batch job may be requeued due to preemption before being held (0 = unlimited)"),
