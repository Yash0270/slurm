...
extern int pack_ctld_job_step_info(void *x, void *arg)
{
	...
	if (args->proto_version >= SLURM_26_05_PROTOCOL_VERSION) {
			      ...
			      uint16_t protocol_version)
{
	 ...
	 * report the original values supplied by the user and the
	 * an intermediate value that might be set by our scheduling
	 * logic to keep scontrol show job consistent through restarts
	 * (e.g. to enforce a partition, association or QOS limit).
	 *
			     ...
			     buf_t *buffer)
{
	...
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		if (step_ptr->switch_step) {
			pack8(1, buffer);
			switch_g_stepinfo_pack(step_ptr->switch_step, buffer,
					       protocol_version);
		} else
			pack8(0, buffer);
			   ...
			   uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		if (uint8_tmp &&
		    (switch_g_stepinfo_unpack(&switch_tmp, buffer,
					      protocol_version)))
			goto unpack_error;
			     ...
			     uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		safe_unpack32(&min_cpus, buffer);
		orig_min_cpus = min_cpus;
		safe_unpack32(&max_cpus, buffer);
		orig_max_cpus = max_cpus;
		safe_unpack32(&min_nodes, buffer);
		safe_unpack32(&max_nodes, buffer);
		safe_unpack32(&num_tasks, buffer);
		...
		safe_unpack16(&core_spec, buffer);
		safe_unpack16(&cpus_per_task, buffer);
		orig_cpus_per_task = cpus_per_task;
		safe_unpack32(&task_dist, buffer);
		...
		safe_unpack32(&pn_min_cpus, buffer);
		orig_pn_min_cpus = pn_min_cpus;
		safe_unpack64(&pn_min_memory, buffer);
		orig_pn_min_memory = pn_min_memory;
		safe_unpack16(&oom_kill_step, buffer);
		safe_unpack32(&pn_min_tmp_disk, buffer);
	...
	job_ptr->details->orig_cpus_per_task = orig_cpus_per_task;
				   ...
				   uint16_t protocol_version)
{
	if (protocol_version >= SLURM_26_11_PROTOCOL_VERSION) {
		...
		/*
		 * Cause-aware requeue counters are controller-internal: packed
		 * only for state save, never in the client job_info RPC. The
		 * unpack side is unconditional, which is safe because only
		 * for_state buffers ever reach job_record_unpack_common().
		 */
		if (for_state) {
			   ...
			   uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		(void) gres_job_state_pack(dump_job_ptr->gres_list_alloc,
					   buffer, dump_job_ptr->job_id, true,
					   protocol_version);
			     ...
			     uint16_t protocol_version)
{
	...
	*out = job_ptr;
	...
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		if (count != NO_VAL) {
			...
			if (count) {
				job_ptr->array_recs->task_id_bitmap =
					bit_alloc(count);
				safe_unpackstr(&job_ptr->array_recs
							->task_id_str,
					       buffer);
		...
		if (switch_g_jobinfo_unpack(&job_ptr->switch_jobinfo, buffer,
					    protocol_version))
			goto unpack_error;
